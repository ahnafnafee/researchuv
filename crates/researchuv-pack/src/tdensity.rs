...
impl TexelDensityParams {
    /// The target density in px per UV unit (unit multiplier applied).
    pub fn density_px_per_unit(&self) -> f64 {
        self.density * self.unit.multiplier()
    }

    /// The scale factor for an island of the given size (max dimension).
    ///
    /// Texel density is defined over the *3D* size: an island representing
    /// `extent` world units should cover `density` px of texture. With a
    /// texture of `tex_size` px over a UV span of `uv_span`, the UV footprint
    /// that yields those pixels is `density·extent·(span/tex)`, so the
    /// uniform scale (UV footprint relative to the unscaled size) is
...
mod tests {
    ...
    fn px_per_meter_scale() {
        let p = TexelDensityParams {
            ...
            density: 100.0, // 100 px/meter → 1e8 px per UV unit (UV = meters)
